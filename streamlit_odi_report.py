...
st.markdown("<p class='subtext'>Type a natural language query. The app will build a chart and generate a colorful PDF report.</p>", unsafe_allow_html=True)
...
if generate and query:
    ...
    # ---- FIX: Kaleido Internal Renderer (No Chrome Needed) ----
    try:
        import kaleido
        pio.defaults.renderers = "png"  # ensure image export works
        pio.kaleido.scope.chromium_executable = None  # use built-in renderer
    except Exception as e:
        st.error(f"⚠️ Kaleido initialization failed: {e}")
        st.stop()
    ...
    try:
        ...
        st.error(f"⚠️ Error saving chart image: {e}")
        st.stop()
