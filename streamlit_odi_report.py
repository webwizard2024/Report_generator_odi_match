...
import os
import unicodedata
from io import BytesIO  # ✅ For in-memory chart
...
if generate and query:
    ...
    if y == "count":
        ...
        if y not in df.columns:
            st.error(f"⚠️ Column '{y}' not found in dataset. Use 'count' for counting.")
            st.stop()
    ...
    try:
        if chart_type.lower() == "pie":
            fig = px.pie(data, names=x, values=value_col, title=query, color_discrete_sequence=px.colors.qualitative.Set3)
        else:
            fig = px.bar(data, x=x, y=value_col, title=query, color=x if value_col=="count" else None,
                         color_discrete_sequence=px.colors.qualitative.Set3, text_auto=True)
    except Exception as e:
        st.error(f"Error creating chart: {e}")
        st.stop()

    # Step 4: Convert chart to PNG (no Chrome required)
    try:
        img_bytes = fig.to_image(format="png", scale=2)
        chart_stream = BytesIO(img_bytes)
    except Exception as e:
    ...
    pdf.ln(4)
    pdf.image(chart_stream, x=30, w=150)  # ✅ Use BytesIO stream directly
